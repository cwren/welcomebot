...
from signalbot import DataMessageHandler, DataMessageContext, SendMessage
...
class CNCCommand(DataMessageHandler):
...
""")
    ...
    def _get_group_info(self, context):
        group_info = [ { 'name': group.name, 'internal_id': group.internal_id } for group in context.bot.groups ]
    ...
    def _get_group_lookup(self, context):
        group_info = self._get_group_info(context)
    ...
    async def handle_data_message(self, context: DataMessageContext) -> None:
        ...
        if group != self.cnc:  # guard against DMs
            self.logger.info("cnc ignoring message not in the CNC group chat")
            return
        
        if not self.store.has_group(group):
            await update_group(self.logger, context.bot, group, self.store)
        ...
        try:
            ...
            await context.send(SendMessage(text=("Sorry, I can't handle overlapping styles")))
            return
        except UnknownStyle as e:
            self.logger.info(f'cnc encountered {e}')
            await context.send(SendMessage(text=(f"Sorry, I don't know how to use {e}")))
            return
        ...
        match(ops[0].lower()):
            ...
            case 'list_reminders':
                ...
                if reminders:
                    gl = self._get_group_lookup(context)
                    reply = 'reminders:\n'
                    reply += '\n'.join(
            ...
            case 'get_reminder':
                ...
                if reminder:
                    ...
                    await context.send(SendMessage(text=(f'could not find reminder {id}')))
                return
